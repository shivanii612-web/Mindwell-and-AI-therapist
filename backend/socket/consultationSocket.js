    ...
    io.on('connection', (socket) => {
        ...
        socket.on('join_consultation_room', async ({ appointmentId } = {}) => {
            try {
                ...
                // If user is a patient, notify the therapist via in-session event (if any, otherwise none)
                if (userRole === 'user') {
                    const therapistId = appointment.therapistId;
                    if (therapistId) {
                        const therapistIdStr = therapistId.toString();
                        // Real-time peer-to-peer event can still be sent, but database notification is removed
                        io.to(`consultation:${appointmentId}`).emit('peer_joined', { userId, role: userRole, userName });
                    }
                }
        ...
        socket.on('send_consultation_message', async ({ appointmentId, message } = {}) => {
            try {
                ...
                // Persist to MongoDB
                const mappedRole = userRole === 'user' ? 'patient' : userRole;
