...
export const JournalPage: React.FC = () => {
  ...
  const { data: journalsRaw = [], refetch } = useGetJournalsQuery({ limit: 50 });
  ...
  const groupByDate = (journals: typeof filteredJournals) => {
    ...
    journals.forEach((journal) => {
      ...
      let key = safeFormatDate(journal.created_at, 'MMMM d, yyyy');
      if (isToday(date)) key = 'Today';
      else if (isYesterday(date)) key = 'Yesterday';
      else if (isThisWeek(date)) key = 'This Week';
      else if (safeFormatDate(date, 'yyyy') === safeFormatDate(new Date(), 'yyyy'))
        key = safeFormatDate(date, 'MMMM');
      else key = safeFormatDate(date, 'MMMM yyyy');
      ...
      <GlassCard className="p-4" hover={false}>
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <option value="all" className="bg-white dark:bg-calm-900 text-calm-800 dark:text-white">All Moods</option>
              <option value="great" className="bg-white dark:bg-calm-900 text-calm-800 dark:text-white">Great</option>
              <option value="good" className="bg-white dark:bg-calm-900 text-calm-800 dark:text-white">Good</option>
              <option value="neutral" className="bg-white dark:bg-calm-900 text-calm-800 dark:text-white">Neutral</option>
              <option value="low" className="bg-white dark:bg-calm-900 text-calm-800 dark:text-white">Low</option>
              <option value="bad" className="bg-white dark:bg-calm-900 text-calm-800 dark:text-white">Bad</option>
            </select>
          </div>
        </div>
      </GlassCard>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-calm-500 mt-1">
                            {safeFormatDate(journal.created_at, 'h:mm a')}
                          </p>
                        </div>
                        {journal.mood && (
                          <Badge
                ...
                <div className="flex items-start justify-between gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-sm text-calm-500">
                      {safeFormatDate(selectedJournal.created_at, 'MMMM d, yyyy • h:mm a')}
                    </p>
                  </div>
                  <Badge
                    variant={
