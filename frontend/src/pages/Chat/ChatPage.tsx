...
export const ChatPage: React.FC = () => {
  ...
  const [sessionId] = useState(() => {
    const userId = user?.id || (user as any)?._id || 'anonymous';
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch(joinUrl(API_URL, '/chat'), {
        ...
        body: JSON.stringify({
          userId: user?.id || (user as any)?._id || 'anonymous',
          sessionId,
  ...
  return (
    <div className="h-[calc(100vh-80px)] flex flex-col overflow-hidden w-full">
      <GlassCard className="flex-1 flex flex-col overflow-hidden" hover={false}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 p-6 space-y-4 custom-scrollbar">
          {messages.filter(m => m.role === 'user').length === 0 && (
            <div className="p-6 mb-4 rounded-2xl bg-lavender-500/5 dark:bg-white/5 border border-dashed border-lavender-500/20 dark:border-white/10 text-center max-w-sm mx-auto space-y-1 animate-pulse">
              <p className="text-sm font-bold text-calm-850 dark:text-white">Start your first conversation.</p>
              <p className="text-xs text-calm-500">The AI will learn your emotional patterns over time.</p>
            </div>
          )}
          <AnimatePresence>
