...
const PricingPage: React.FC = () => {
    ...
    // Only show completed payments originally processed via Razorpay (contains razorpayOrderId)
    const realPaymentHistory = paymentHistory.filter((p: any) => p.status === 'completed' && !!p.razorpayOrderId);
                    ...
                    <div className="flex-1">
                        ...
                        {subscriptionEndDate && (
                            <p className="text-sm text-emerald-600 dark:text-emerald-400">
                                Renews on {safeFormatDate(subscriptionEndDate, 'MMMM d, yyyy')}
                            </p>
                        )}
                    </div>
            ...
            <GlassCard className="p-6" hover={false}>
                ...
                {realPaymentHistory.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {realPaymentHistory.map((payment: any) => (
                            <div
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 mt-0.5">
                                            <p className="text-xs text-calm-400">
                                                {payment.paidAt
                                                    ? safeFormatDate(payment.paidAt, 'MMM d, yyyy • h:mm a')
                                                    : payment.createdAt
                                                        ? safeFormatDate(payment.createdAt, 'MMM d, yyyy • h:mm a')
                                                        : '—'}
                                            </p>
                                        ...
                                        {payment.subscriptionEndDate && (
                                            <p className="text-[10px] text-emerald-500 mt-0.5">
                                                Valid until {safeFormatDate(payment.subscriptionEndDate, 'MMM d, yyyy')}
                                            </p>
                                        )}
                                    </div>
                                </div>
