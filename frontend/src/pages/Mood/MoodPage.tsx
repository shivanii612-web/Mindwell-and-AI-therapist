...
export const MoodPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-8 w-full px-6 md:px-8 pt-0">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Badge variant="primary" size="lg" className="bg-lavender-500/10 text-lavender-600 border border-lavender-200 dark:border-lavender-500/20">
            ...
            {moods.length > 0 ? `Last Sync: ${safeFormatDate(moods[0].logged_at, 'h:mm a')}` : 'No Entries Yet'}
          </Badge>
          <Avatar
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-12 xl:col-span-8 space-y-6">
          ...
          >
            <GlassCard className="p-8 bg-white/90 dark:bg-calm-950/40 border-lavender-500/20 dark:border-white/5 shadow-xl" hover={false}>
              <div className="flex items-center gap-2 mb-8" id="mood-log-heading">
        ...
        <div className="lg:col-span-12 xl:col-span-4 space-y-6">
          {moods.length > 0 ? (
              ...
              >
                <GlassCard className="p-6 bg-white/90 dark:bg-calm-950/40 border-lavender-500/20 dark:border-white/5 shadow-xl" hover={false}>
                  ...
                  <div className="grid grid-cols-7 gap-1 text-[10px] text-calm-400 font-black mb-4 uppercase">
                    {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, index) => <div key={`${d}-${index}`} className="text-center">{d}</div>)}
                  </div>
                          ...
                          >
                            ...
                            {dayMood && (
                              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-calm-900 text-white text-[10px] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20 shadow-2xl pointer-events-none text-center">
                                ...
                                <p className="text-calm-400 text-[8px] font-black uppercase tracking-tighter">{safeFormatDate(dayMood.logged_at, 'MMM d, yyyy')}</p>
                              </div>
                            )}
                          </div>
                        </div>
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-[10px] font-bold text-calm-400">
                                  {safeFormatDate(mood.logged_at, 'EEEE, MMM d · h:mm a')}
                                </p>
                              </div>
                            </div>
                            <Badge className="bg-lavender-500/10 text-lavender-600 border-none font-black text-[10px]">
            ...
            <GlassCard className="p-8 text-center bg-white/90 dark:bg-calm-950/40 border-lavender-500/20 dark:border-white/5 shadow-xl flex flex-col items-center justify-center space-y-6 h-full min-h-[350px]" hover={false}>
              <div className="text-6xl drop-shadow-md animate-bounce">😊</div>
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-calm-900 dark:text-white">No mood entries yet.</h3>
                <p className="text-sm text-calm-600 dark:text-calm-400 max-w-sm mx-auto leading-relaxed font-medium">
                  Track how you're feeling today to begin understanding your emotional patterns.
                </p>
              </div>
              <button
                onClick={() => {
                  document.getElementById('mood-log-heading')?.scrollIntoView({ behavior: 'smooth' });
                }}
                className="px-6 py-3 rounded-full bg-gradient-to-r from-[#7c3aed] to-[#4f46e5] hover:from-[#8b5cf6] hover:to-[#6366f1] text-white font-bold text-sm shadow-md transition-all hover:shadow-[0_0_15px_rgba(124,58,237,0.3)] cursor-pointer"
              >
                Record Today's Mood
              </button>
            </GlassCard>
