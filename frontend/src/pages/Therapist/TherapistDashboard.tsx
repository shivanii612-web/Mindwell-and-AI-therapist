...
import {
    Calendar, Clock, CheckCircle, XCircle, Check, User, FileText,
    Video, MessageSquare, TrendingUp, Search, Filter,
    Plus, MoreHorizontal, Settings, Shield
} from 'lucide-react';
...
import { connectConsultationSocket } from '@lib/consultationSocket';
...
export const TherapistDashboard: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'dashboard' | 'consultation' | 'appointments' | 'availability' | 'notes'>('dashboard');
    const [pending, setPending] = useState<Appointment[]>([]);
    const [mine, setMine] = useState<Appointment[]>([]);
    const [loading, setLoading] = useState(true);
    const [actionLoading, setActionLoading] = useState<Record<string, boolean>>({});
    const notesInput: Record<string, string> = {};

    // Read tab from query string e.g. /therapist?tab=appointments
    useEffect(() => {
        const params = new URLSearchParams(location.search);
        const tabParam = params.get('tab');
        if (tabParam && ['dashboard', 'consultation', 'appointments', 'availability', 'notes'].includes(tabParam)) {
                    ...
                    >
                        ...
                        <GlassCard className="p-8" hover={false}>
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-bold text-calm-800 dark:text-white">Recent Session Requests</h3>
                                <button onClick={() => setActiveTab('appointments')} className="text-sm font-bold text-lavender-500 hover:text-lavender-600 transition-colors">View All</button>
                            </div>
                            ...
                            <div className="space-y-4">
                                {loading ? (
                                    ...
                                    return (
                                        <div key={apt._id} className="p-4 rounded-3xl bg-calm-100/50 dark:bg-white/5 border border-calm-200/80 dark:border-white/10 hover:border-lavender-500/30 transition-all flex items-center justify-between flex-wrap gap-4 group">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-lavender-500/20 to-accent-500/20 flex items-center justify-center text-lavender-600 dark:text-lavender-400 font-bold group-hover:scale-105 transition-transform">
                                                    {user.full_name[0]?.toUpperCase()}
                                                </div>
                                                <div>
                                                    <div className="flex items-center gap-2">
                                                        <h4 className="font-bold text-calm-800 dark:text-white">{user.full_name}</h4>
                                                        <Badge variant={isPending ? 'warning' : (apt.status === 'Accepted' || apt.status === 'Completed') ? 'success' : 'error'} size="sm">{apt.status}</Badge>
                                                    </div>
                                                    <p className="text-xs text-calm-500 dark:text-calm-400">{apt.sessionType} • {safeFormatDate(apt.preferredDate, 'MMM d, h:mm a')}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                {isPending ? (
                                                    <>
                                                        <button
                                                            ...
                                                            className="px-4 py-2 rounded-xl bg-[#16a34a] hover:bg-[#15803d] active:bg-[#166534] text-white font-extrabold text-xs shadow-md border border-[#15803d]/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                                                        >
                                                            <Check className="w-3.5 h-3.5 text-white" />
                                                            {actionLoading[apt._id] ? 'Accepting...' : 'Accept'}
                                                        </button>
                                                        <button
                                                            onClick={() => handleAction(apt._id, 'reject')}
                                                            disabled={actionLoading[apt._id]}
                                                            className="px-4 py-2 rounded-xl bg-red-100 hover:bg-red-200 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-300 dark:border-red-500/30 font-bold text-xs transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                                                        >
                                                            <XCircle className="w-3.5 h-3.5" />
                                                            Reject
                                                        </button>
                                                    </>
                                                ) : isAccepted && apt.sessionStatus !== 'ended' && (
                                                    apt.sessionStatus === 'live' ? (
                                                        <div className="flex flex-col gap-1 items-end">
                                                            <button
                                                                onClick={() => navigate(`/consultation?type=${apt.sessionType.toLowerCase()}&appointmentId=${apt._id}`)}
                                                                className="px-4 py-2 rounded-xl bg-[#16a34a] hover:bg-[#15803d] text-white font-bold text-xs transition-all flex items-center gap-1 shadow-md"
                                                            >
                                                                <Video className="w-3 h-3 text-white" /> Join Session
                                                            </button>
                                                            <button
                        ...
                        <GlassCard className="p-8" hover={false}>
                            ...
                            <div className="space-y-6">
                                {[...pending, ...mine].sort((a, b) => new Date(b.preferredDate).getTime() - new Date(a.preferredDate).getTime()).map((apt) => {
                                    ...
                                    return (
                                        <div key={apt._id} className="p-6 rounded-3xl bg-calm-100/50 dark:bg-white/5 border border-calm-200/80 dark:border-white/10 hover:border-lavender-500/30 transition-all">
                                            <div className="flex items-center justify-between flex-wrap gap-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-calm-500 dark:text-calm-400">{user.email}</p>
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <Badge variant={isPending ? 'warning' : (apt.status === 'Accepted' && apt.sessionStatus !== 'ended') ? 'success' : 'error'} size="sm">{apt.sessionStatus === 'ended' ? 'Session Ended' : apt.status}</Badge>
                                                ...
                                                <div className="flex items-center gap-6">
                                                    <div className="text-right">
                                                        <p className="text-sm font-bold text-calm-700 dark:text-calm-200">{safeFormatDate(apt.preferredDate, 'EEEE, MMM d')}</p>
                                                        ...
                                                        {isPending ? (
                                                            <>
                                                                <button
                                                                    onClick={() => handleAction(apt._id, 'accept')}
                                                                    disabled={actionLoading[apt._id]}
                                                                    className="px-5 py-2.5 rounded-xl bg-[#16a34a] hover:bg-[#15803d] active:bg-[#166534] text-white font-extrabold text-sm shadow-md border border-[#15803d]/50 transition-all focus:outline-none focus:ring-2 focus:ring-green-500/50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                                                >
                                                                ...
                                                                <button
                                                                    onClick={() => handleAction(apt._id, 'reject')}
                                                                    disabled={actionLoading[apt._id]}
                                                                    className="px-5 py-2.5 rounded-xl bg-red-100 hover:bg-red-200 active:bg-red-300 dark:bg-red-500/10 dark:hover:bg-red-500/20 dark:active:bg-red-500/30 text-red-600 dark:text-red-400 border border-red-300 dark:border-red-500/30 font-bold text-sm transition-all focus:outline-none focus:ring-2 focus:ring-red-500/40 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                                                                >
                                                            ...
                                                            apt.sessionStatus === 'live' ? (
                                                                <div className="flex flex-col gap-1 items-end">
                                                                    <button
                                                                        onClick={() => navigate(`/consultation?type=${apt.sessionType.toLowerCase()}&appointmentId=${apt._id}`)}
                                                                        className="px-6 py-2.5 rounded-xl bg-[#16a34a] hover:bg-[#15803d] text-white font-bold text-sm shadow-lg flex items-center gap-2"
                                                                    >
                                        ...
                                        <div key={apt._id} className="p-6 rounded-3xl bg-calm-50 dark:bg-white/5 border border-calm-100 dark:border-white/10 group hover:border-lavender-500/20 transition-all">
                                            <div className="flex items-center justify-between mb-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <h4 className="font-bold text-calm-800 dark:text-white">{user.full_name}</h4>
                                                        <p className="text-xs text-calm-400">{safeFormatDate(apt.preferredDate, 'MMMM do, yyyy')}</p>
                                                    </div>
                                                </div>
