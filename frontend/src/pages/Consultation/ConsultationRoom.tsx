...
import {
    ...
    Volume2, Play, Square, Save, CheckCircle, AlertCircle, WifiOff, Stethoscope
} from 'lucide-react';
...
export const ConsultationRoom: React.FC = () => {
    ...
    const userId = (profile?.id || (profile as any)?._id || '').toString();
    ...
    const appointmentRef = useRef<AppointmentInfo | null>(null);
    useEffect(() => {
        ...
        if (appointment) {
            ...
            const therapistName =
                appointment?.therapistId?.full_name ||
    ...
    const normalizeConsultationMessage = useCallback((raw: any): Message => {
        // Extract senderId as a plain string — handles both populated object and raw ObjectId
        const rawSenderId =
            raw.senderId?._id?.toString() ||
            raw.senderId?.id?.toString() ||
            raw.senderId?.toString() ||
            '';

        // Preserve the senderRole exactly as stored/sent by the server.
        // The render loop maps 'user' → patient display. Do NOT re-resolve here
        // because appointment may not be loaded yet when early messages arrive.
        const rawSenderRole = raw.senderRole || raw.senderId?.role || '';

        // senderName stored on the message is a reliable denormalized value from the server.
        const rawSenderName = raw.senderName || raw.senderId?.full_name || '';

        const messageId = raw._id?.toString() || raw.id?.toString() || String(Date.now() + Math.random());

        const currentUserId = userId;
        const isOwn = !!(rawSenderId && currentUserId && rawSenderId.toLowerCase() === currentUserId.toLowerCase());

        return {
            id: messageId,
            senderId: rawSenderId,
            senderName: rawSenderName,
            senderRole: rawSenderRole,
            text: raw.message || raw.text || '',
            isOwn,
            time: new Date(raw.createdAt || Date.now()).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit',
            }),
        };
    ...
    const normalizeRef = useRef(normalizeConsultationMessage);
    useEffect(() => {
        normalizeRef.current = normalizeConsultationMessage;
    ...
    useEffect(() => {
        ...
        const onHistory = ({ messages: history }: { appointmentId: string; messages: any[] }) => {
            ...
            if (Array.isArray(history)) {
                const normalized = history.map(raw => normalizeRef.current(raw));
    ...
    // Listen for therapist starting/ending/resetting the session in real time.
    ...
    useEffect(() => {
        ...
        // session_reset: therapist undid an accidental start → back to waiting
        const onSessionReset = ({ appointmentId: aid }: { appointmentId: string }) => {
            if (aid === appointmentId) {
                setSessionStatus('pending_session');
                leaveCall();
                toast('Session was reset by the therapist.', { id: 'session-reset' });
            }
        };
    ...
    const endSessionCall = async () => {
        try {
            if (appointmentId && token) {
                await fetch(joinUrl(API_URL, `/appointments/${appointmentId}/end-session`), {
                    method: 'PUT',
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
        } catch (e) {
            console.error('MindWell: Error ending session:', e);
        } finally {
            leaveCall();
    ...
    return (
        <div className="h-[calc(100vh-6rem)] flex flex-col gap-4 max-w-7xl mx-auto">
            {/* ─── Session Header ─── */}
                                ...
                                <motion.div key="video" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col">
                                    ...
                                    {dailyJoined && dailyRoomUrl && (
                                        <div className="flex-1 flex flex-col">
                                            ...
                                            <div className="flex-none flex justify-center py-3 bg-calm-950/90 backdrop-blur">
                                                <button onClick={endSessionCall} className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-red-500 text-white font-bold text-sm hover:bg-red-600 transition-all shadow-lg">
                                                    <PhoneOff className="w-4 h-4" /> {isTherapist ? 'End Session' : 'Leave Call'}
                                                </button>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {localJoined && !dailyJoined && (
                                        <div className="flex-1 relative bg-calm-950 overflow-hidden">
                                            ...
                                            <div className="absolute bottom-4 right-4 z-20 w-44 h-32 rounded-2xl bg-calm-900 border-2 border-white/20 shadow-2xl overflow-hidden flex items-center justify-center">
                                                {localCamOff || permissionDenied ? (
                                                    <div className="flex flex-col items-center gap-1 text-calm-400">
                                                        <User className="w-8 h-8" />
                                                        <span className="text-[10px]">You ({localCamOff ? 'Cam Off' : 'No Mic'})</span>
                                                    </div>
                                                ) : (
                                                    <video
                                                        ref={videoRef}
                                            ...
                                            {/* Controls toolbar */}
                                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 px-6 py-3 rounded-2xl bg-calm-900/90 backdrop-blur border border-white/10 shadow-2xl">
                                                {!permissionDenied && (
                                                    <>
                                                        <button
                                                            ...
                                                            className={cn(
                                                                ...
                                                                localIsMuted
                                                                    ...
                                                                    : 'bg-calm-800 text-calm-200 hover:bg-calm-700'
                                                            )}
                                                        >
                                                ...
                                                >
                                                    <PhoneOff className="w-4 h-4" /> {isTherapist ? 'End Session' : 'Leave'}
                                                </button>
                                            </div>
                                        </div>
                                    )}
                                            ...
                                            <div className="flex-1 overflow-y-auto p-6 space-y-4 flex flex-col w-full">
                                                ...
                                                {messages.map((msg) => {
                                                    const isOwnMessage =
                                                        !!userId &&
                                                        !!msg.senderId &&
                                                        String(msg.senderId).toLowerCase() === userId.toLowerCase();

                                                    const isTherapistMsg = msg.senderRole === 'therapist';

                                                    return (
                                                        <div
                                                            key={msg.id}
                                                            className={cn(
                                                                'flex max-w-[80%] w-full',
                                                                isOwnMessage ? 'self-end justify-end' : 'self-start justify-start'
                                                            )}
                                                        >
                                                            <div className={cn('flex flex-col space-y-1', isOwnMessage ? 'items-end' : 'items-start')}>
                                                                {/* Message bubble — color by role, corner by ownership */}
                                                                <div
                                                                    className={cn(
                                                                        'px-4 py-3 rounded-2xl text-sm font-medium leading-relaxed shadow-sm border',
                                                                        isOwnMessage
                                                                            ? isTherapistMsg
                                                                                ? 'bg-gradient-to-br from-teal-500 to-emerald-500 text-white rounded-tr-none border-transparent'
                                                                                : 'bg-gradient-to-br from-lavender-500 to-accent-500 text-white rounded-tr-none border-transparent'
                                                                            : isTherapistMsg
                                                                                ? 'bg-teal-50 dark:bg-teal-950/20 text-teal-800 dark:text-teal-200 rounded-tl-none border-teal-200/50 dark:border-teal-900/50'
                                                                                : 'bg-lavender-50 dark:bg-lavender-950/20 text-lavender-800 dark:text-lavender-200 rounded-tl-none border-lavender-200/50 dark:border-lavender-900/50'
                                                                    )}
                                                                >
                                                                    {msg.text}
                                                                </div>
